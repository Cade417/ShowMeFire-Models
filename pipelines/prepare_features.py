...
def prepare_features():
    df = pd.read_csv(paths.TRAINING_DATA_DIR / 'training_set_mo.csv')
    ...
    df.to_csv(paths.TRAINING_DATA_DIR / 'ai_features_mo.csv', index=False)
    ...
    plt.ylabel('Moisture %')
    plt.legend()
    plt.savefig(paths.PLOTS_DIR / 'station_preview.png')

def enhance_features_with_lags():
    df = pd.read_csv(paths.TRAINING_DATA_DIR / 'ai_features_mo.csv')
    df['obs_time'] = pd.to_datetime(df['obs_time'])
    ...
    df.to_csv(paths.TRAINING_DATA_DIR / 'final_training_data.csv', index=False)
