"""Canonical precipitation decoding for cached forecast datasets.

All public helpers return millimeters.  HRRR/Herbie ``tp`` and ``APCP`` are
accumulations since model initialization; interval amounts are derived by
differencing adjacent forecast leads rather than summing accumulations.
"""
...
def normalize_to_mm(values, units: str | None):
    """Normalize supported precipitation units to millimeters, failing closed."""
...
def decode_forecast_precipitation(dataset: xr.Dataset) -> DecodedPrecipitation:
    ...
    if dimension is None:
        ...
        if len(leads) != millimeters.sizes[dimension]:
            raise ValueError("precipitation lead coordinate does not match data")
        durations = np.diff(np.concatenate(([0.0], leads))).astype(float)
        if np.any(durations < 0):
            raise ValueError("precipitation leads are not chronological")
        duration = xr.DataArray(durations, dims=(dimension,), coords={dimension: millimeters[dimension]})
        shape = [1] * millimeters.ndim
        shape[millimeters.get_axis_num(dimension)] = len(durations)
        duration = duration.broadcast_like(millimeters)
    ...
    return DecodedPrecipitation(
        ...
        reset_flag=reset,
        partial_window_flag=partial,
        variable_name=name,
        source_units=str(units),
        accumulation_kind="cumulative_since_init" if accumulated else "interval",
    )
