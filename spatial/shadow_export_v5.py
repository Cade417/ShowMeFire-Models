"""Finalize a V5 directory for explicit, non-registry API shadow use."""
...
def main():
    ...
    args = parser.parse_args(); output = args.candidate_dir / "shadow_bundle_manifest.json"
    if output.exists(): raise SystemExit(f"Refusing to overwrite {output}")
    check_api_copy(); contract = json.loads((args.candidate_dir / "contract.json").read_text())
    if contract.get("status") != "experimental_unregistered" or contract.get("beta_registration_allowed"):
        raise SystemExit("V5 candidate is not an unregistered experiment")
    names = ("base_xgboost.json", "specialist_xgboost.json", "guard.json", "uncertainty.json", "contract.json")
    value = {"status": "experimental_shadow_only", "registry_channel": None, "beta_registration_allowed": False,
             ...
             "assets": {name: sha256_file(args.candidate_dir / name) for name in names}}
