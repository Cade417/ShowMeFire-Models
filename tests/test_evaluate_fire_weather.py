...
def _synthetic_panel(n_rows: int = 600, seed: int = 0) -> pd.DataFrame:
    ...
    # kbdi/gdd_accum carry real signal the "weather-only" baseline can't see,
    # so the candidate should measurably beat it - the point of this gate.
    frame[model_bundle.DEFAULT_LABEL_COLUMN] = (
        frame["wind_ms"] * 1.2 + frame["kbdi"] * 0.6 + frame["gdd_accum"] * 0.4
        - frame["fm10_pct"] * 0.9 + rng.normal(0, 0.3, size=n_rows)
    ).clip(lower=0.0)
    return frame
...
class BuildReportTests(unittest.TestCase):
    def test_report_has_every_expected_gate_with_no_silent_omission(self):
        ...
        self.assertEqual(gate_names, {
            "sufficient_test_rows", "beats_naive_weather_only_baseline",
    ...
    def test_candidate_with_real_kbdi_gdd_signal_beats_the_weather_only_baseline(self):
        report = evaluate.build_report(_synthetic_panel(n_rows=800))
        baseline_gate = next(g for g in report["gates"] if g["name"] == "beats_naive_weather_only_baseline")
        self.assertEqual(baseline_gate["status"], "pass")
